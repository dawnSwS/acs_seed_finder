...
impl ShadowPipeline {
    ...
    fn get_cpu_neighbor(key: i32, out_keys: &mut [u32; 8]) -> usize {
        let mut count = 0; let dirs = [6, 4, 7, 5, 1, 2, 3, 0];
        for &dir in &dirs {
            let mut n = -1;
            if dir == 0 { n = key + 192; if n <= 0 || n >= 36864 { n = -1; } } else if dir == 1 { n = key - 192; if n <= 0 || n >= 36864 { n = -1; } }
            else if dir == 2 { n = key - 1; if n < 0 || n >= 36864 || (key/192) != (n/192) { n = -1; } } else if dir == 3 { n = key + 1; if n < 0 || n >= 36864 || (key/192) != (n/192) { n = -1; } }
    ...
    fn cpu_out_line(&mut self, src: usize, t_target: usize, w: i32, lv: i32, maxcount: i32, ctype: CType) {
        ...
        let limit = ((w * 2 + 1) * (w * 2 + 1)) as usize; let base_around_50 = get_base_around_50(); let mut current_max = maxcount;
        for word in (0..1152).rev() {
            let mut mask = self.bb[T_TMP_1][word]; if word == 0 { mask &= !1; }
            while mask != 0 {
                let bit = 31 - mask.leading_zeros(); mask ^= 1 << bit;
                let key = word * 32 + bit as usize; let cx = (key % 192) as i32; let cy = (key / 192) as i32;
                for i in 0..limit {
                    if i >= base_around_50.len() { break; }
                    let (dx, dy) = base_around_50[i]; let nx = cx + dx; let ny = cy + dy;
                    if nx >= 0 && nx < 192 && ny >= 0 && ny < 192 {
                        if self.rand.random_range_int(0, 100, RandomType::EmNone, "OutLine") > lv { continue; }
                        let nw = (ny * 192 + nx) as usize / 32; let n_bit = 1 << ((ny * 192 + nx) as usize % 32);
    ...
    fn cpu_random_line_from_mine_dir(&mut self, w: i32, size: i32, def: usize, ctype: CType) {
        ...
        for _ in 0..size {
            ...
            let mut f_key = key; if n_count > 0 { f_key = v_keys[self.rand.random_range_int(0, n_count as i32, RandomType::EmNone, "MakeMap") as usize]; }
            let nw = (f_key / 32) as usize; let nbit = 1 << (f_key % 32);
        ...
        for _ in 0..ecount {
            if flag {
                for w in 0..1152 {
                    let mut b = 0;
                    while b < 32 {
                        let mask = self.bb[def][w] >> b; if mask == 0 { break; } b += mask.trailing_zeros(); 
                        ...
                        if self.rand.random_range_int(0, 100, RandomType::EmNone, "MakeMap") <= expand_lv {
                            ...
                            for d in 0..n_count {
                                let nk = n_keys[d] as usize; let nw = nk / 32; let n_bit = 1 << (nk % 32);
                ...
                for w in (0..1152).rev() {
                    ...
                    while b >= 0 {
                        ...
                        b = 31 - mask.leading_zeros() as i32; let key = w * 32 + b as usize;
                        if self.rand.random_range_int(0, 100, RandomType::EmNone, "MakeMap") <= expand_lv {
                            let mut n_keys = [0u32; 8]; let n_count = Self::get_cpu_neighbor(key as i32, &mut n_keys);
                            for d in 0..n_count {
                                let nk = n_keys[d] as usize; let nw = nk / 32; let n_bit = 1 << (nk % 32);
    ...
    fn step_1_cpu(&mut self) {
        ...
        for _ in 0..3 {
            let mut i = 0; let mut num = self.rand.random_range_int(0, 192, RandomType::EmNone, "MakeMineDir");
        ...
        let kx = self.rand.random_range_int(76, 133, RandomType::EmNone, "MakeMap"); let ky = self.rand.random_range_int(114, 133, RandomType::EmNone, "MakeMap");
    ...
    fn step_2_cpu_b(&mut self) {
        ...
        for i in 0..4 {
            let num2 = self.rand.random_range_int(57, 115, RandomType::EmNone, "MakeMap"); let mut j = 0;
            while j < self.rand.random_range_int(5, 15, RandomType::EmNone, "MakeMap") {
                let mut k = -1; let ny_nx = num2 + j;
    ...
    fn step_7_cpu(&mut self) {
        self.cpu_out_line(T_ROCK_BROWN, T_STONE_LAND, 1, 30, 0, CType::CheckCon2); self.cpu_out_line(T_IRON_ORE, T_STONE_LAND, 1, 30, 0, CType::CheckCon2); self.cpu_out_line(T_SILVER_ORE, T_STONE_LAND, 1, 30, 0, CType::CheckCon2); self.cpu_out_line(T_COPPER_ORE, T_STONE_LAND, 1, 30, 0, CType::CheckCon2); self.cpu_out_line(T_ROCK_BROWN, T_STONE_LAND, 1, 30, 0, CType::CheckCon2);
...
fn optimize_pass(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var y = 0u; y < 192u; y++) {
        ...
        for (var xw = 0u; xw < 6u; xw++) {
            let w = b + xw; let mask = buf_b[w]; var f_mask = 0u;
            if (mask != 0u) {
                let c = cr[xw]; var cl = c >> 1u; if (xw < 5u) { cl |= (cr[xw+1u] << 31u); } var cr_b = c << 1u; if (xw > 0u) { cr_b |= (cr[xw-1u] >> 31u); }
                var u = pr[xw]; if (y == 1u && xw == 0u) { u &= 0xFFFFFFFEu; }
                var ul = u >> 1u; if (xw < 5u) { ul |= (pr[xw+1u] << 31u); } var ur = u << 1u; if (xw > 0u) { ur |= (pr[xw-1u] >> 31u); }
                var d = nr[xw]; var dl = d >> 1u; if (xw < 5u) { dl |= (nr[xw+1u] << 31u); } var dr = d << 1u; if (xw > 0u) { dr |= (nr[xw-1u] >> 31u); }
...
fn apply_tmp(@builtin(global_invocation_id) id: vec3<u32>) {
    let task = id.x; if (task >= cfg.batch_size) { return; }
    let offset = task * 1152u; for (var i = 0u; i < 1152u; i++) { buf_a[offset + i] |= buf_out[offset + i]; }
}
@compute @workgroup_size(64)
    ...
    let mut all_results: Vec<(i32, usize)> = chunks.into_par_iter().flat_map(|current_start| {
        ...
        let run_gpu_pass = |cmd: &str, radius: u32, opt_min: u32, opt_max: u32, read_back: bool| -> Option<Vec<u32>> {
            ...
            if read_back {
                ...
                let copy_size = if cmd == "count" { 
                    (diff * 4) as u64 
                } else { 
                    diff as u64 * 1152 * 4 
                };
                encoder.copy_buffer_to_buffer(read_src, 0, &buffers.readback_buf, 0, copy_size); queue.submit(Some(encoder.finish()));
                let slice = buffers.readback_buf.slice(0..copy_size); let (tx, rx) = std::sync::mpsc::channel();
                slice.map_async(wgpu::MapMode::Read, move |v| tx.send(v).unwrap());
                while rx.try_recv().is_err() { std::thread::yield_now(); }
                
                let data = slice.get_mapped_range(); let out_data: &[u32] = bytemuck::cast_slice(&data); let res = out_data.to_vec();
                drop(data); buffers.readback_buf.unmap(); Some(res)
            } else { queue.submit(Some(encoder.finish())); None }
        };
        ...
        run_opt(&mut engines, T_FERTILE_SOIL, 5, 3, 1, CType::AllTrue);
        
        let ss = 3;
        for e in engines.iter_mut() {
            e.cpu_random_and_expand(T_D_DEPTH_WATER, ss as i32 - 1, 2 * ss as i32 - 1, 13 + 6 * ss as i32, CType::NoBorn, CType::NoBorn);
        }
        run_opt(&mut engines, T_D_DEPTH_WATER, 5, 3, 1, CType::NoBorn);
        ...
        run_opt(&mut engines, T_SHALLOW_WATER, 5, 3, 1, CType::NoBorn);
        ...
        run_opt(&mut engines, T_LING_SOIL, 5, 3, 1, CType::CheckCon);
